...
const NEGATIVE_RELATION_OPS = new Set(['none', 'isNot'])
  ...
  function processRelationFilter(
    ...
    for (const [op, opValue] of Object.entries(value)) {
      ...
      if (NEGATIVE_RELATION_OPS.has(op) && nested.schema && hasWhereForced(nested.forced)) {
        throw new ShapeError(
          `Relation filter "${key}.${op}" on model "${model}" mixes client-controlled and forced conditions. ` +
            `Under negative relation operators (none, isNot), merging weakens the filter. ` +
            `Either move forced conditions to a separate top-level "${op}" branch, ` +
            `or make all conditions under this operator client-controlled or all forced.`,
        )
      }
