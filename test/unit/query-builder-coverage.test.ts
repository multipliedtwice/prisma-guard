...
function makeQb() {
  return createQueryBuilder(TYPE_MAP, ENUM_MAP)
}
...
describe('query-builder coverage: nested include args', () => {
  ...
  it('throws on _count in include with unknown relation', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', {
      include: {
        _count: { select: { nonexistent: true } },
      },
    })).toThrow(ShapeError)
  })
...
describe('query-builder coverage: nested select args', () => {
  ...
  it('throws on nested select args for scalar field', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', {
      select: {
        email: { select: { something: true } } as any,
      },
    })).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: cursor and orderBy errors', () => {
  ...
  it('throws on relation field in orderBy', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', {
      orderBy: { company: true },
    })).toThrow(ShapeError)
  })

  it('throws on Json field in orderBy', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', {
      orderBy: { metadata: true },
    })).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: where errors', () => {
  ...
  it('throws on relation field in where', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', {
      where: { company: { equals: true } },
    })).toThrow(ShapeError)
  })
  ...
  it('throws on unknown model', () => {
    expect(() => qb.buildQuerySchema('NonExistent', 'findMany', {
