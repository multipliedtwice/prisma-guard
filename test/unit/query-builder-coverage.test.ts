...
const TYPE_MAP: TypeMap = {
  User: {
    ...
    metadata: { type: 'Json', isList: false, isRequired: false, isId: false, isRelation: false, hasDefault: false, isUpdatedAt: false },
...
function makeQb() { return createQueryBuilder(TYPE_MAP, ENUM_MAP) }

describe('query-builder coverage: aggregate operations', () => {
  const qb = makeQb()

  it('builds _count with specific fields', () => {
    const schema = qb.buildQuerySchema('User', 'aggregate', { _count: { email: true, name: true } })
  ...
  it('builds _avg with valid numeric fields', () => {
    const schema = qb.buildQuerySchema('User', 'aggregate', { _avg: { age: true, score: true } })
  ...
  it('throws on relation field in _avg', () => {
    expect(() => qb.buildQuerySchema('User', 'aggregate', { _avg: { company: true } })).toThrow(ShapeError)
  })

  it('builds _sum, _min, _max with valid fields', () => {
    const schema = qb.buildQuerySchema('User', 'aggregate', { _sum: { age: true }, _min: { age: true, score: true }, _max: { score: true } })
    const result = schema.parse({ _sum: { age: true }, _min: { age: true }, _max: { score: true } })
  ...
  it('throws on unknown field in _sum', () => {
    expect(() => qb.buildQuerySchema('User', 'aggregate', { _sum: { nonexistent: true } })).toThrow(ShapeError)
  })

  it('throws on relation field in _min', () => {
    expect(() => qb.buildQuerySchema('User', 'aggregate', { _min: { posts: true } })).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: groupBy', () => {
  ...
  it('builds groupBy with by and _count', () => {
    const schema = qb.buildQuerySchema('User', 'groupBy', { by: ['role'], _count: true })
    const result = schema.parse({ by: ['role'], _count: true })
    expect(result.by).toEqual(['role'])
  })

  it('throws on groupBy without by', () => {
    expect(() => qb.buildQuerySchema('User', 'groupBy', { _count: true } as any)).toThrow(ShapeError)
  })
  ...
  it('throws on relation field in by', () => {
    expect(() => qb.buildQuerySchema('User', 'groupBy', { by: ['company'] })).toThrow(ShapeError)
  })

  it('throws on groupBy with include', () => {
    expect(() => qb.buildQuerySchema('User', 'groupBy', { by: ['role'], include: { posts: true } } as any)).toThrow(ShapeError)
  })

  it('throws on groupBy with select', () => {
    expect(() => qb.buildQuerySchema('User', 'groupBy', { by: ['role'], select: { id: true } } as any)).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: aggregate method validation', () => {
  ...
  it('throws on aggregate with include', () => {
    expect(() => qb.buildQuerySchema('User', 'aggregate', { include: { posts: true } } as any)).toThrow(ShapeError)
  })

  it('throws on aggregate with select', () => {
    expect(() => qb.buildQuerySchema('User', 'aggregate', { select: { id: true } } as any)).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: forced tree merge on include', () => {
  ...
  it('merges forced WHERE into nested include when client sends object', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', {
      include: { posts: { where: { title: { contains: true }, userId: { equals: 'forced-user' } } } },
    })
    const result = schema.parse({ include: { posts: { where: { title: { contains: 'test' } } } } })
    const postsWhere = (result.include as any).posts.where
    expect(postsWhere).toEqual({ AND: [{ title: { contains: 'test' } }, { userId: { equals: 'forced-user' } }] })
  })
  ...
  it('preserves forced values when client does not include the relation', () => {
    ...
    const result = schema.parse({ include: {} })
    expect((result.include as any).posts).toBeUndefined()
  })
})
...
describe('query-builder coverage: forced tree merge on select', () => {
  ...
  it('merges forced WHERE into nested select relation', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', {
      select: { id: true, posts: { where: { title: { contains: true }, userId: { equals: 'forced-user' } }, select: { title: true } } },
    })
    const result = schema.parse({
      select: { id: true, posts: { where: { title: { contains: 'hello' } }, select: { title: true } } },
    })
    const postsWhere = (result.select as any).posts.where
    expect(postsWhere).toEqual({ AND: [{ title: { contains: 'hello' } }, { userId: { equals: 'forced-user' } }] })
  })

  it('expands true to forced values in select mode', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', {
      select: { id: true, posts: { where: { userId: { equals: 'forced-user' } }, select: { title: true } } },
    })
...
describe('query-builder coverage: nested include args', () => {
  ...
  it('builds nested include with orderBy, cursor, take, skip', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', {
      include: { posts: { orderBy: { title: true }, cursor: { id: true }, take: { max: 50, default: 10 }, skip: true } },
    })
    const result = schema.parse({
      include: { posts: { orderBy: { title: 'asc' }, cursor: { id: 'cursor-id' }, take: 20, skip: 5 } },
    })
    const posts = (result.include as any).posts
  ...
  it('throws on nested include with both select and include', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', {
      include: { posts: { select: { id: true }, include: { author: true } } as any },
    })).toThrow(ShapeError)
  })

  it('builds _count in include as true', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', { include: { _count: true } })
  ...
  it('builds _count in include with select', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', { include: { _count: { select: { posts: true } } } })
    const result = schema.parse({ include: { _count: { select: { posts: true } } } })
    expect((result.include as any)._count).toEqual({ select: { posts: true } })
  })

  it('throws on _count in include with unknown relation', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { include: { _count: { select: { nonexistent: true } } } })).toThrow(ShapeError)
  })

  it('throws on _count in include with non-relation field', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { include: { _count: { select: { email: true } } } })).toThrow(ShapeError)
  })

  it('throws on _count in include with invalid shape', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { include: { _count: { notSelect: { posts: true } } } as any })).toThrow(ShapeError)
  })

  it('throws on _count in include with empty select', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { include: { _count: { select: {} } } })).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: nested select args', () => {
  ...
  it('builds nested select with orderBy, cursor, take, skip', () => {
    const schema = qb.buildQuerySchema('User', 'findMany', {
      select: { id: true, posts: { select: { id: true, title: true }, orderBy: { title: true }, cursor: { id: true }, take: { max: 50, default: 10 }, skip: true } },
    })
    const result = schema.parse({
      select: { id: true, posts: { select: { id: true }, orderBy: { title: 'desc' }, cursor: { id: 'c1' }, take: 5, skip: 2 } },
    })
    const posts = (result.select as any).posts
...
describe('query-builder coverage: caller with context function', () => {
  ...
  it('throws when context missing for caller shape function', () => {
    const schema = qb.buildQuerySchema<{ companyId: string }>('User', 'findMany', {
      '/admin': (ctx) => ({ where: { companyId: { equals: ctx.companyId } } }),
    })
...
describe('query-builder coverage: method arg validation', () => {
  ...
  it('throws on disallowed arg for method', () => {
    expect(() => qb.buildQuerySchema('User', 'count', { include: { posts: true } } as any)).toThrow(ShapeError)
  })

  it('throws on both include and select', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { include: { posts: true }, select: { id: true } } as any)).toThrow(ShapeError)
  })

  it('throws on take default exceeding max', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { take: { max: 10, default: 20 } })).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: cursor and orderBy errors', () => {
  ...
  it('throws on unknown field in cursor', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { cursor: { nonexistent: true } })).toThrow(ShapeError)
  })

  it('throws on relation field in cursor', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { cursor: { company: true } })).toThrow(ShapeError)
  })

  it('throws on relation field in orderBy', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { orderBy: { company: true } })).toThrow(ShapeError)
  })

  it('throws on Json field in orderBy', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { orderBy: { metadata: true } })).toThrow(ShapeError)
  })
})
...
describe('query-builder coverage: where errors', () => {
  ...
  it('throws on Json field in where', () => {
    expect(() => qb.buildQuerySchema('User', 'findMany', { where: { metadata: { equals: true } } })).toThrow(ShapeError)
  })

  it('throws on unknown model', () => {
    expect(() => qb.buildQuerySchema('NonExistent', 'findMany', { where: { id: { equals: true } } })).toThrow(ShapeError)
  })
})