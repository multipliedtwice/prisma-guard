...
describe("single static shape", () => {
  ...
  it("rejects unknown fields in body", () => {
    ...
    expect(() =>
      schema.parse({ where: { email: { contains: "x" } }, extra: true }),
    ).toThrow();
  });
...
describe("include shapes", () => {
  ...
  it("accepts nested include with where", () => {
    ...
    expect((result.include as any).posts.where).toEqual({
...
describe("where validation", () => {
  it("now supports relation field in where via relation operators", () => {
    expect(() =>
      qb().buildShapeZodSchema("User", "findMany", {
...
describe("method validation", () => {
  it("rejects include on count", () => {
    expect(() =>
      qb().buildQuerySchema("User", "count", { include: { posts: true } }),
  ...
  it("allows select + include in shape, rejects in body", () => {
    const schema = qb().buildQuerySchema("User", "findMany", {
      select: { id: true },
      include: { posts: true },
    });
