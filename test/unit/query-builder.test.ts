...
describe('orderBy shapes', () => {
  ...
  it('throws on relation field in orderBy', () => {
    expect(() =>
      qb().buildQuerySchema('User', 'findMany', {
        orderBy: { posts: true } as any,
      }),
    ).toThrow('Relation field')
  })

  it('throws on Json field in orderBy', () => {
    expect(() =>
      qb().buildQuerySchema('User', 'findMany', {
        orderBy: { data: true } as any,
      }),
    ).toThrow('Json field')
  })
})
...
describe('method validation', () => {
  ...
  it('rejects take on findUnique', () => {
    expect(() =>
      qb().buildQuerySchema('User', 'findUnique', {
        take: { max: 10, default: 5 },
      }),
    ).toThrow('not allowed for method "findUnique"')
  })
...
describe('caller map', () => {
  it('routes to correct shape by caller', () => {
    ...
    const r1 = schema.parse({ caller: '/admin', where: { email: { contains: 'x' } } })
    expect(r1.where).toEqual({ email: { contains: 'x' } })

    const r2 = schema.parse({ caller: '/public', where: { age: { gte: 18 } } })
  ...
  it('strips caller from body before parsing', () => {
    const schema = qb().buildQuerySchema('User', 'findMany', {
      '/admin': { where: { email: { contains: true } } },
    })
    const result = schema.parse({ caller: '/admin', where: { email: { contains: 'x' } } })
  ...
  it('caller map with context function', () => {
    ...
    const result = schema.parse(
      { caller: '/admin', where: { email: { contains: 'x' } } },
      { ctx: { tenantId: 't1' } },
    )
    expect(result.where).toEqual({
      AND: [
...
describe('isShapeConfig heuristic fix', () => {
  ...
  it('throws when caller map key collides with reserved keys', () => {
    ...
    ).toThrow('collides with reserved shape config key')
  })
